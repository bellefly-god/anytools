...
Object.entries(translations).forEach(([id, contentEn]) => {
  // 查找提示词块
  ...
  if (match) {
    ...
    if (!block.includes('contentEn:')) {
      // 在 content: `...` 后面添加 contentEn
      const contentPattern = new RegExp(`(id: '${id}',[\\s\\S]*?content: \\\`[^\\\`]+\\\`,)`, 'g');
      content = content.replace(contentPattern, `$1\n    contentEn: \`${contentEn.replace(/\\\`/g, '\\\\`')}\`,`);
      count++;
      console.log('✅', id);
    } else {
